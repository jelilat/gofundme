...
const baseMsgWithdrawDonation = { creator: "", id: 0 };
export const MsgWithdrawDonation = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgWithdrawDonation };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
...
const baseMsgWithdrawDonationResponse = {};
...
export class MsgClientImpl {
    ...
    WithdrawDonation(request) {
        const data = MsgWithdrawDonation.encode(request).finish();
        const promise = this.rpc.request("cosmonaut.gofundme.gofundme.Msg", "WithdrawDonation", data);
        return promise.then((data) => MsgWithdrawDonationResponse.decode(new Reader(data)));
    }
}
var globalThis = (() => {
